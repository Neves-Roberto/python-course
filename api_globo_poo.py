...
class apiGlobo:
    ...
    '''
    Descritivo desta classe
    '''

    def __init__(self):
        #print('carregado o construtor')

        self.status_api = "STATUS"
        self.hoje = datetime.now()
        self.hoje_formatado = str(self.hoje.year) + '-' + str(self.hoje.month) + '-' + str(self.hoje.day)
        self.grant_type = 'password'
        self.username = 'entregadigital.api'
        self.password = 'Tri#01'
        self.client_id = 'downloadapp'
        self.client_secret = 'downloadsecret'
        self.url_token = "https://api.tvglobo.com.br/integradorgmid/oauth/token"
    ...
    def GetMateriais(self,destino='SAN', idPlayers='1,2,3,4', statusDownload='', dataDe=[], dataAte=[],
        ...
        # statusDownload ['entregue','emAndamento','pendente',]
        ...
        if cdMateriais == ['']:
            dados_materiais = {'destino': destino, 'idPlayers': idPlayers, 'statusDownload': statusDownload,
                               'dataDe': dataDe, 'dataAte': dataAte}
        else:
            dados_materiais = {'destino': destino, 'idPlayers': idPlayers, 'statusDownload': statusDownload,
                               'dataDe': dataDe, 'dataAte': dataAte, 'cdMateriais[]': cdMateriais}
        ...
        #print('data no formato AAAA-MM-DD DATADE ')
        #print(dataDe)
        #print('data no formato AAAA-MM-DD DATAATE ')
        #print(dataAte)
    ...
    def GetLimites(self,idRequisicao='1'):
    ...
    def GetDestinos(self,idRequisicao='1'):
    ...
    def GetPlayers(self,idRequisicao='1'):
    ...
    def GetEnderecos(self,idRequisicao='1', idMateriais=''):
    ...
    def DownloadMaterial(self,idMaterial, nomeArquivo, md5):
    ...
    def GerarPaginaEstrutura(self):
        # TODO montar função de gera arquivo html contendo conteudo da pagina equivalente da consulta realizada
        self.itens = (
            'ID_MATERIAL', 'PTO_VENDA', 'COD_MATERIAL', 'PLAYER', 'DATA_ENVIO', 'TITULO', 'STATUS_DOWNLOAD',
            'NOME_ARQUIVO',
            'MD5', 'URL_DOWNLOAD')

        with open('teste.html', "w") as pagina:
            self.hora_atual = self.hoje_formatado + ' ' + str(self.hoje.hour) + ':' + str(self.hoje.minute) + ':' + str(self.hoje.second)
            ...
            .coluna8  {background:#F99;}
                    ...
                    <h3>Ultima atualização ''' + self.hora_atual + '''</h3>
                    <table>
            ...
            for self.item in self.itens:
                pagina.write("            <th>{0}</th>".format(self.item))
                print("            <th>{0}</th>".format(self.item))

            pagina.write('''   </tr>
            <tr>''')
            print('''   </tr>''')

    def GerarPaginaCorpo(self,iten, AtivarLinha, DesativaLinha):
...
def GerarListaMateriais(datade, dataate, Materiais):
    ...
    for listaMaterial in ListaMateriais:
        ...
        for iten in (
        'idMaterial', 'ptoVenda', 'codMaterial', 'player', 'dataEnvio', 'titulo', 'statusDownload', 'nomeArquivo',
        'md5'):
            # espacoPadrao = 2
            # 1 espaço
            ...
            if iten == 'md5':
                ...
                try:
                    ...
                    print(url)
                    GerarPaginaCorpo("<a href=" + url + ">" + listaMaterial['nomeArquivo'] + "</a>", AtivarLinha=Ativar,
                                     DesativaLinha=Desativar)
                except:
                    print("Não foi possivel obter a URL de Download")
                    GerarPaginaCorpo("Não foi possivel obter a URL de Download", AtivarLinha=Ativar,
                                     DesativaLinha=Desativar)
