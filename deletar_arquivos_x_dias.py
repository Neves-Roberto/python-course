...
DIAS = 95 # arqvuivos com mais de X dias serao deletados


data_hoje_log = str(datetime.datetime.now().day) + '-' + str(datetime.datetime.now().month) + '-' + str(
datetime.datetime.now().year)
data_de = datetime.datetime.strptime(
    str(datetime.datetime.now().year) + '-' + str(datetime.datetime.now().month) + '-' + str(
        datetime.datetime.now().day), "%Y-%m-%d")
...
print(timestamp)
contador = 0
for arquivo in lista_arquivos(path, 'mxf'):
...
arquivo_log.close()
#deletar_arquivos_x_dias(path,path_api,DIAS,extensao='mxf')
