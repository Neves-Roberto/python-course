...
def extrai_campos_importantes(json_id):
    ...
    if (json_id != "erro"):
        ...
        arquivo_json = open('Z:\json\{0}.json'.format(teste['name']), 'w')
...
def extrai_campos_importantes_arquivo(json_id):
    # retorna lista contendo o que é importante
    cidade_json = 'Z:\json\{0}.json'.format(dicionario_ids_cidades[json_id])
...
def construir_planilha():
    ...
    # -----------------------------------
    workbook_build.save('Z:\clima_tempo_python\clima_tempo_python.xls')


def main(lista_ids_cidades=lista_ids_cidades):
    # função principal
    # path
    caminho = "i:\\clima_tempo\\realistic\\200px\\"
    extensao = ".png"
    graus = "º"
    print("Versão 1.3")
    time.sleep(5)
    ...
    read_planilha = xlrd.open_workbook('Z:\clima_tempo_python\clima_tempo_python.xls')
        ...
        for i in lista_ids_cidades[0:10]:
            ...
            print(
                'Cidade {0} Temperatura: {1} Icone: {2}'.format(dados_importantes[0], str(dados_importantes[1]) + graus,
                                                                caminho + dados_importantes[2] + extensao))
            ...
            w_sheet.write(indice - 1, 2, caminho + dados_importantes[2] + extensao)
    ...
    workbook_edit.save('Z:\clima_tempo_python\clima_tempo_python.xls')
    time.sleep(5)
